...
Open cmd and run these commands, or save this into a `shade.bat` file and execute using cmd. You must locate `shade_backend.exe`. 

- If you've installed it to your user this should be in `C:\Users\<YOUR USER FOLDER>\AppData\Local\Programs\shade\resources\extraResources\server\`
...
start /B "" "C:\Users\%USERNAME%\AppData\Local\Programs\shade\resources\extraResources\server\shade_backend.exe"
```
...
Open powershell and run these commands, or save this into a `shade.ps1` file and execute using powershell. You must locate `shade_backend.exe`. 
